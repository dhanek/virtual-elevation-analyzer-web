...
- **Project parity validation chain (run during Plan 03-02 and Plan 03-03):**
  ...
  - `bash scripts/validate-ui-shell-guardrails.sh --ci-only` → exit 0; the CI parity chain remains green after shell extraction (Plan 03-03 closeout PASSED).
  - `cd frontend && npm run build` → exit 0; production bundle still compiles with the extracted standard VE shell modules (Plan 03-02 PASSED, 412 kB bundle).
...
- **Negative-case regression signal:** If BEHV-02 regresses, the first failure indicators would be a non-zero exit from `bash scripts/validate-ui-shell-guardrails.sh --ci-only`, a broken `npm run build`, or manual checklist failure where standard VE no longer renders/updates correctly after FIT/CSV load; no such failure signals were observed in the cited Plan 03-02/03-03 evidence.
...
| BEHV-02 | User can run standard virtual elevation analysis after shell extraction with unchanged visible behavior and analysis outputs | completed | Standard VE render path preserved through `handleAnalyze` → `createModeRenderCallbacks` → `showVirtualElevationAnalysisInline` (extracted shell module) with no behavior change in plot builders or analysis math. Regression contract anchors: `docs/testing/ui-shell-regression-contract.md` `## File-load navigation` and `## CI checkpoint baseline`. Manual parity surface: `docs/testing/ui-shell-manual-checklist.md` `## FIT and CSV file-load navigation`. Verification commands: `bash scripts/validate-ui-shell-guardrails.sh --ci-only` (Plan 03-03 PASSED) and `cd frontend && npm run build` (Plan 03-02 PASSED, 412 kB bundle). Includes explicit negative-case regression criteria in `### Key Link Verification → BEHV-02 parity-depth references`. | - |

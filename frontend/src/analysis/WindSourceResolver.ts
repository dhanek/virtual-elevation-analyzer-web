...
export interface ResolveWindSeriesInput {
	fitData: Pick<
		ActivityDataLike,
		"timestamps" | "air_speed" | "wind_speed" | "wind_yaw"
	>;
...
export function getDefaultWindSource(
	fitData: Pick<ActivityDataLike, "air_speed" | "wind_speed">,
): WindSource {
	const airSpeed = toNumberArray(fitData.air_speed);
	const windSpeed = toNumberArray(fitData.wind_speed);
	return hasUsableValues(airSpeed) || hasUsableValues(windSpeed)
		? "fit"
		: "constant";
}

export function resolveWindSeries(
	input: ResolveWindSeriesInput,
): WindSeriesResolution {
	...
	const requestedWindSource =
		normalizeWindSource(input.windSource) ??
		getDefaultWindSource(input.fitData);
	const selectedWindSource =
		requestedWindSource === "compare" ? "fit" : requestedWindSource;
	...
	const offsetSeconds =
		input.params?.air_speed_offset ?? (dataSource === "air_speed" ? 2 : 0);
	...
	if ((input.airSpeedCalibrationPercent ?? 0) !== 0) {
		const calibrationMultiplier =
			1 + (input.airSpeedCalibrationPercent ?? 0) / 100;
		resolvedWindSpeed = resolvedWindSpeed.map(
			(speed) => speed * calibrationMultiplier,
		);
	}
...
export function applyAirSpeedOffset(
	airSpeed: number[],
	...
	for (let i = 0; i < airSpeed.length; i++) {
		...
		result[i] =
			sourceIndex >= 0 && sourceIndex < airSpeed.length
				? airSpeed[sourceIndex]
				: NaN;
	}
...
function resolveYawArray(
	windYaw: ArrayLike<number> | undefined | null,
	length: number,
): number[] {
...
function normalizeWindSource(
	value: WindSource | string | null | undefined,
): WindSource | null {
	switch (value) {
		case "constant":
		case "fit":

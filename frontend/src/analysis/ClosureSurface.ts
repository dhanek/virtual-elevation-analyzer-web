...
export function surfaceFromZ(
	...
	const cda = refineAxis(cdaValues, minI, (index) => z[minJ][index]);
	const crr = refineAxis(crrValues, minJ, (index) => z[index][minI]);
...
export function closureBand(
	...
	if (!(toleranceM > 0)) {
		throw new Error(
			`closure band tolerance must be positive, got ${toleranceM}`,
		);
	}

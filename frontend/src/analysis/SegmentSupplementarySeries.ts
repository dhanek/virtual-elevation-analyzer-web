...
export interface BuildSegmentSupplementarySeriesInput {
	...
	params: Pick<
		AnalysisParameters,
		"wind_speed" | "wind_direction" | "wind_height_factor"
	>;
...
export function buildSegmentSupplementarySeries(
	...
	const apparentWindSpeedMps =
		input.selectedWindSource === "constant"
...
export function calculateConstantApparentWindSeries(
	...
	const configuredWindSpeed =
		resolveAppliedWindSpeed(params, params.wind_speed ?? 0) ?? 0;
	if (configuredWindSpeed === 0) {
		return [...velocity];
	}

	if (
		positionLat.length < 2 ||
		positionLong.length < 2 ||
	...
	return velocity.map((groundSpeed, index) => {
		...
		const effectiveWind =
			configuredWindSpeed * Math.cos((angleDiff * Math.PI) / 180);
...
export function buildVirtualDistanceSeries(
	...
	for (let i = 1; i < timestamps.length; i++) {
		...
		const apparentSpeed =
			Number.isFinite(apparentWindSpeedMps[i]) && apparentWindSpeedMps[i] > 0
				? apparentWindSpeedMps[i]
				: 0;
		const groundSpeed =
			Number.isFinite(velocity[i]) && velocity[i] > 0 ? velocity[i] : 0;

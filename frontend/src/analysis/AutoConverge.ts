...
import { judgeRidge, type RidgeColumn } from "./ClosureRidge";
import {
	commonDistanceGrid,
	PROFILE_FLAT_REASON,
...
type SolveStatus = "ok" | "clamped-low" | "clamped-high";
...
export function usableSegments(
	...
	const crr = (bounds.crrMin + bounds.crrMax) / 2;
...
export function solveBoth(
	...
	for (let i = 0; i < RIDGE_SAMPLES; i++) {
		...
		ridge.push({
			error: Math.sqrt(sumOfSquares),
			onRidge: crr.status === "ok",
		});
	}

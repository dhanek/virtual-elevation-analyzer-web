...
describe("poolClosureSurface pooling", () => {
	...
	test("rejects a degenerate grid", () => {
		expect(() => poolClosureSurface([], [0.3], [0.005])).toThrow(/2x2/);
	});
});
...
describe("grid sizing", () => {
	test("defaults to 100 and drops to 41 above the measured window budget", () => {
		...
		expect(chooseGridSteps(COARSE_GRID_WINDOW_SAMPLES)).toBe(
			DEFAULT_GRID_STEPS,
		);
		expect(chooseGridSteps(COARSE_GRID_WINDOW_SAMPLES + 1)).toBe(
			COARSE_GRID_STEPS,
		);
	});

...
const REQUIRED_DISCLAIMERS: Array<{ label: string; pattern: RegExp }> = [
	{
		label: "the browser display of the four-plot compare view",
		pattern: /four-plot[\s\S]*browser|browser[\s\S]*four-plot/i,
	},
	{
		label: "the D-16 DevTools rows G-05, G-06, G-11, G-12",
		pattern: /G-05[\s\S]*G-06[\s\S]*G-11[\s\S]*G-12/,
	},
	{
		label: "the D-09 entry (t) out-and-back impression",
		pattern: /D-09 entry \(t\)/,
	},
	{
		label: "the D-09 (a)/(c)/(d) real-ride before/after numbers",
		pattern: /D-09 entries \(a\), \(c\) and \(d\)/,
	},
];
 ...
 */
function expectSequencesEqual(
	actual: number[],
	expected: number[],
	label: string,
): void {
	expect(`${label}: length ${actual.length}`).toBe(
		`${label}: length ${expected.length}`,
	);
...
describe.skipIf(!fixturePresent)("synthetic out-and-back fixture", () => {
	describe("Block A — the geometry really is out-and-back", () => {
		test("the fixture is one contiguous index space of the pinned size", () => {
			...
			for (const name of FIXTURE_ARRAYS) {
				expect(`${name}: ${json[name].length}`).toBe(
					`${name}: ${json.record_count}`,
				);
			}
		});
		...
		test("the sections tile the whole array with no gap and no overlap", () => {
			...
			expect(sections.map((section) => section.sectionNumber)).toEqual(
			...
			for (const section of sections) {
				expect(
					`section ${section.sectionNumber} starts at ${section.outboundStartIdx}`,
				).toBe(`section ${section.sectionNumber} starts at ${cursor}`);
				expect(
					`section ${section.sectionNumber} inbound starts at ${section.inboundStartIdx}`,
				).toBe(
					`section ${section.sectionNumber} inbound starts at ${section.outboundEndIdx + 1}`,
				);
				cursor = section.inboundEndIdx + 1;
			}
			expect(`sections cover ${cursor} samples`).toBe(
				`sections cover ${json.record_count} samples`,
			);
		});
		...
		test("distance never goes backwards across the whole fixture", () => {
			...
			const firstDescent = json.distance.findIndex(
				(value, i) => i > 0 && value < json.distance[i - 1],
			);
		...
		describe.each(EXPECTED_SECTION_NUMBERS)("section %i", (sectionNumber) => {
			const section = () => {
				const found = loadOutAndBackRide().sections.find(
			...
			const legs = (name: (typeof FIXTURE_ARRAYS)[number]) => {
				...
				return {
					...
					inboundReversed: json[name]
						.slice(s.inboundStartIdx, s.inboundEndIdx + 1)
						.reverse(),
				};
			};
			...
			test("the inbound leg retraces the outbound latitude sequence", () => {
				...
				expectSequencesEqual(
					inboundReversed,
					outbound,
					`section ${sectionNumber} position_lat`,
				);
			});
			...
			test("the inbound leg retraces the outbound longitude sequence", () => {
				...
				expectSequencesEqual(
					inboundReversed,
					outbound,
					`section ${sectionNumber} position_long`,
				);
			});
			...
			test("the inbound leg retraces the outbound altitude profile", () => {
				...
				expectSequencesEqual(
					inboundReversed,
					outbound,
					`section ${sectionNumber} altitude`,
				);
			});
			...
			test("the inbound leg's wind yaw is the reversed outbound yaw rotated 180 degrees", () => {
				...
				expectSequencesEqual(
					inboundReversed,
					expected,
					`section ${sectionNumber} wind_yaw`,
				);
			});
			...
			test("distance increases across the leg turn rather than jumping backwards", () => {
				...
				const s = section();
				const atTurn =
					json.distance[s.inboundStartIdx] - json.distance[s.outboundEndIdx];

				expect(
					`section ${sectionNumber} turn step positive: ${atTurn > 0}`,
				).toBe(`section ${sectionNumber} turn step positive: true`);
			});
		});
	});
	...
	describe("Block B — the fixture keeps declaring its own evidentiary limits", () => {
		test("it declares itself synthetic and deterministic, and names its regeneration command", () => {
			...
			expect(provenance.regenerateWith).toContain(
				"build-out-and-back-fixture.ts",
			);
		});
		...
		test.each(REQUIRED_DISCLAIMERS)(
			"whatThisDoesNotDischarge names $label",
			({ label, pattern }) => {
				const { provenance } = loadOutAndBackRide();
				...
				expect(
					`whatThisDoesNotDischarge (${entries.length} entries) names ${label}: ${matched}`,
				).toBe(
	...
	describe("Block C — the loader hands back what the file carries", () => {
		test("the sensor channels come through unmodified", () => {
			...
			for (const name of [
				...
				expectSequencesEqual(
					fitData[name],
					json[name as (typeof FIXTURE_ARRAYS)[number]],
					`fitData.${name}`,
				);
			}
		...
		test("the never-stored channels are zero-filled to full length", () => {
			...
			for (const name of [
				...
				expect(
					`${name}: ${values.length} samples, ${values.filter((v) => v !== 0).length} non-zero`,
				).toBe(`${name}: ${EXPECTED_RECORD_COUNT} samples, 0 non-zero`);
			}
		});
	});
});

...
const SETTINGS_FORMAT = "virtual-elevation-analyzer-settings";
...
export type ParsedSettingsEnvelope =
	{ ok: true; envelope: SettingsEnvelope } | { ok: false; error: string };
...
export function parseSettingsEnvelope(text: string): ParsedSettingsEnvelope {
	...
	if (candidate.format !== SETTINGS_FORMAT) {
		return {
			ok: false,
			error: "The file is not a Virtual Elevation Analyzer settings export.",
		};
	}

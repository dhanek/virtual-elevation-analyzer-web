	...
	function reference(
		...
		for (const group of groups.values()) {
			const profiles = group.map((segment) =>
				GRID.map(
					(_, i) => segment.p0[i] + dc * segment.jc[i] + dr * segment.jr[i],
				),
			);
			const meanTarget =
				group.reduce((sum, segment) => sum + segment.target, 0) / group.length;
			const mean = GRID.map(
				(_, i) =>
					profiles.reduce((sum, profile) => sum + profile[i], 0) / group.length,
			);

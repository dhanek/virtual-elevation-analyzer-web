...
import type { FitData, LapData, ParsingStatistics } from '@wasm/virtual_elevation_analyzer.js';
...
export type ActivityDataLike = (FitData & {
    humidity?: number[];
    pressure?: number[];
    cda_reference?: number[];
}) | ActivityData;
export type ActivityLapLike = LapData | ActivityLap;
export type ActivityParsingStatisticsLike = ParsingStatistics | ActivityParsingStatistics;
...
export function createSelectedSlice(input: {
    startIdx: number;
    endIdx: number;
...
export class AppState {
    ...
    set currentFitResult(result: ActivityResult | null) {
        ...
        if (this.activity.loadedActivity && result) {
            ...
            this.activity.loadedActivity.parsingStatistics = result.parsing_statistics;
        }
    }

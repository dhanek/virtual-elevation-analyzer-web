 ...
 */
function computeAwsTileGeoref(z: number, x: number, y: number, tileSize: number = 512): { worldFile: string; prjFile: string } {
    ...
    const prjFile = 'PROJCS["WGS 84 / Pseudo-Mercator",GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",6378137,298.257223563]],PRIMEM["Greenwich",0],UNIT["degree",0.0174532925199433]],PROJECTION["Mercator_1SP"],PARAMETER["central_meridian",0],PARAMETER["scale_factor",1],PARAMETER["false_easting",0],PARAMETER["false_northing",0],UNIT["metre",1]]';
...
export class MultiDEMManager {
    ...
    private awsTileManagers: { manager: DEMManager; z: number; x: number; y: number }[] = [];
    private awsZoom: number = 12;

    /**
     * Load a remote DEM result into the coordinator
     */
    async loadSource(source: DEMSourceType, result: RemoteDEMResult): Promise<void> {
    ...
    async correctAllSources(
        ...
        if (otoMgr?.isDEMLoaded()) {
            try {
                const correction = await otoMgr.correctElevation(lats, lons, fallbackAltitudes);
                results.set('opentopography', {
                    source: 'opentopography',
                    elevations: correction.elevations,
    ...
    private correctWithAWSTiles(
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
                elevations[i] = fallbackAltitudes[i];
                errorCount++;
                continue;
            }
            ...
            if (mgr?.isDEMLoaded()) {
                try {
                    // Use batch_lookup for single point via the underlying processor
                    const result = mgr.singleLookup(lat, lon);
        ...
        // Fix leading zeros (same as DEMManager.correctElevation)
        let firstValidIdx = -1;
        let firstValidValue = 0;
        for (let i = 0; i < elevations.length; i++) {
            if (!isNaN(elevations[i]) && elevations[i] !== 0) {
                firstValidIdx = i;
                firstValidValue = elevations[i];
                break;
            }
        }
        if (firstValidIdx > 0) {
            for (let i = 0; i < firstValidIdx; i++) {
                if (elevations[i] === 0 || isNaN(elevations[i])) {
                    elevations[i] = firstValidValue;
                }
            }
        }

		...
		for (const demtype of datasetsToTry) {
			...
			try {
				const response = await fetch(
					`${OPENTOPO_BASE_URL}?${params.toString()}`,
				);

				if (!response.ok) {
					const body = await response.text().catch(() => "");
					lastError = new Error(
						`OpenTopography ${demtype} failed (${response.status}): ${body || response.statusText}`,
					);
				...
				log.debug(
					`OpenTopography ${demtype}: received ${(data.length / 1024).toFixed(0)} KB`,
				);
				...
				return {
					...
					metadata: {
						dataset: demtype,
						resolution:
							demtype.includes("90") || demtype === "SRTMGL3" ? "90m" : "30m",
						tileCount: 1,
						bbox: clampedBbox,
					},
				};
...
export class AWSTerrainTilesClient {
	async fetchDEM(
		...
		log.debug(
			`AWS Terrain Tiles: zoom=${zoom}, tiles=${totalTiles} (${minX}-${maxX} x ${minY}-${maxY})`,
		);

		if (totalTiles > 50) {
			throw new Error(
				`Too many AWS tiles needed (${totalTiles}). Try a shorter route or lower zoom.`,
			);
		}
		...
		const results = await Promise.allSettled(
			tileRequests.map(async (tile) => {
				...
				if (!response.ok) {
					throw new Error(
						`AWS tile ${tile.x}/${tile.y} failed: ${response.status}`,
					);
				}
				const arrayBuffer = await response.arrayBuffer();
				completed++;
				onProgress?.(
					"aws-terrain",
					`Fetched ${completed}/${totalTiles} tiles`,
					10 + (completed / totalTiles) * 80,
				);
				return {
	...
	async fetchForRoute(
		...
		if (sources.includes("opentopography")) {
			if (!config.apiKey) {
				...
				promises.push(
					this.openTopoClient
						.fetchDEM(bbox, config.apiKey, config.dataset, onProgress)
						.then((result) => {
							results.set("opentopography", result);
						})
						.catch((err) => {
							log.error("OpenTopography fetch failed:", err);
						}),
				);
			}
		}
		...
		if (sources.includes("aws-terrain")) {
			promises.push(
				this.awsClient
					.fetchDEM(bbox, undefined, onProgress)
					.then((result) => {
						results.set("aws-terrain", result);
					})
					.catch((err) => {
						log.error("AWS Terrain Tiles fetch failed:", err);
					}),
			);
		}

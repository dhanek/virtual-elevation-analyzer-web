...
function forecastAllNullBody(
	slot: string = RECENT_SLOT,
...
function mockFetchSequence(
	...
	const fetchMock = vi.fn(async (_url: string) => {
		...
		if (next === undefined) {
			throw new Error(
				`fetch called ${callIndex} times but only ${queued.length} responses were queued`,
			);
		}
...
describe("WeatherAPI fixtures", () => {
	test("the fetch queue serves responses in order and rejects overruns", async () => {
		...
		await expect(fetch("third")).rejects.toThrow(
			/only 2 responses were queued/,
		);
	...
	test("metadata fixture pins the activity inside the Forecast window", () => {
		...
		const daysAgo = Math.floor(
			(NOW.getTime() - metadata.middleDate.getTime()) / 86_400_000,
		);
		expect(daysAgo).toBe(9);
		const oldDaysAgo = Math.floor(
			(NOW.getTime() - OLD_ACTIVITY.getTime()) / 86_400_000,
		);

 ...
 * coverage.
 */
async function buildDeflatedZip(
	name: string,
	content: Uint8Array,
): Promise<Uint8Array> {
...
describe("buildZip / readZip", () => {
	it("round-trips two entries byte-for-byte", async () => {
		const fit = new Uint8Array([
			0x0e, 0x10, 0x43, 0x08, 0x2e, 0x46, 0x49, 0x54, 7, 0,
		]);
	...
	it("reads a DEFLATED zip, the shape OS archivers produce", async () => {
		...
		const entries = await readZip(
			await buildDeflatedZip("bundle/ride.json", content),
		);
	...
	it("refuses an entry whose bytes fail the CRC check", async () => {
		const zipped = buildZip([
			{ name: "ride.fit", data: text("healthy bytes") },
		]);

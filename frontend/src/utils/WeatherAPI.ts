...
export class WeatherAPI {
	...
	private async fetchFromAPI(
		...
		log.debug("═══════════════════════════════════════════════════════");
		log.debug(
			`🌐 OPEN-METEO ${apiType.toUpperCase()} API REQUEST (${resolution})`,
		);
		log.debug("═══════════════════════════════════════════════════════");
		...
		try {
			...
			if (resolution === "15min") {
				const has15minData = data.minutely_15?.temperature_2m?.some(
				...
				if (has15minData) {
					...
					const hasHourlyData = data.hourly?.temperature_2m?.some(
						(v: number | null) => v !== null,
					);
					if (hasHourlyData) {
						log.debug(
							"⚠️ minutely_15 data is all null, using hourly from same response",
						);
						...
						log.debug(
							"⚠️ Both minutely_15 and hourly data are null in Forecast response",
						);
						return null;
					} else {
			...
			log.debug(
				"⏰ Queried At:",
				new Date(weatherData.queriedAt).toISOString(),
			);
			log.debug("═══════════════════════════════════════════════════════");
	...
	private extractMinutely15Data(
		...
		const pressure =
			hourIndex !== -1 ? data.hourly.surface_pressure?.[hourIndex] : undefined;

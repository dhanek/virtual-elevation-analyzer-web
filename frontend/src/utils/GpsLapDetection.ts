...
export class GpsLapDetector {
		...
		if (groupedPassings.length >= 2) {
			...
			while (lapStart < groupedPassings.length - 1) {
				...
				if (lapEnd !== null) {
					...
					const lapDistance =
						(this.distance[endPassing.index] -
							this.distance[startPassing.index]) /
					...
					const lap: DetectedLap = {
						...
						directionName: this.bearingToCompassDirection(
							startPassing.direction,
						),
			...
			if (i < windowSize) {
				...
				if (i + windowSize < len) {
					bearing = this.calculateBearing(
						this.positionLat[i],
						this.positionLon[i],
						this.positionLat[i + windowSize],
						this.positionLon[i + windowSize],
					);
				} else if (i + 1 < len) {
					bearing = this.calculateBearing(
						this.positionLat[i],
						this.positionLon[i],
						this.positionLat[i + 1],
						this.positionLon[i + 1],
					);
				} else {
				...
				bearing = this.calculateBearing(
					this.positionLat[i - halfWindow],
					this.positionLon[i - halfWindow],
					this.positionLat[i + halfWindow],
					this.positionLon[i + halfWindow],
				);
			}
	...
	private findPassings(bearings: number[]): PassingPoint[] {
		...
		const { markerLat, markerLon, trimStart, trimEnd, proximityThreshold } =
			this.config;

		for (
			let i = trimStart;
			i <= Math.min(trimEnd, this.positionLat.length - 1);
			i++
		) {
	...
	private groupAndClusterPassings(passings: PassingPoint[]): PassingPoint[] {
		...
		for (const passing of passings) {
			...
			if (
				...
				passing.index - currentGroup[currentGroup.length - 1].index <=
					PASSING_CLUSTER_GAP_POINTS
			) {
				currentGroup.push(passing);
			} else {
				// Find closest point in the group
				if (currentGroup.length > 0) {
					const closest = currentGroup.reduce(
...
export class OutAndBackDetector {
				...
				if (marker === "A") {
					...
					if (dirDiff > this.config.angleThreshold) {
						...
						const outboundDuration =
							this.timestamps[currentSection.outboundEndIdx!] -
							this.timestamps[currentSection.outboundStartIdx!];
						const inboundDuration =
							this.timestamps[currentSection.inboundEndIdx!] -
							this.timestamps[currentSection.inboundStartIdx!];

						// Calculate distances
						const outboundDistance =
							(this.distance[currentSection.outboundEndIdx!] -
								this.distance[currentSection.outboundStartIdx!]) /
							METERS_PER_KILOMETER;
						const inboundDistance =
							(this.distance[currentSection.inboundEndIdx!] -
								this.distance[currentSection.inboundStartIdx!]) /
							METERS_PER_KILOMETER;
	 ...
	 */
	private findPassingsNearMarker(
		markerLat: number,
		markerLon: number,
		bearings: number[],
	): PassingPoint[] {

...
export class GibliCsvParser {
	...
	static parse(csvContent: string): GibliCsvData {
		...
		return {
			...
			timeRangeSeconds:
				convertedData.timestamps[convertedData.timestamps.length - 1] -
				convertedData.timestamps[0],
		};
	}
	...
	private static analyzeMetadata(
		...
		GibliCsvData,
		"hasEnvironmentalData" | "hasCdaReference" | "hasLapData" | "hasWindAngle"
	> {
		...
		const hasCdaReference =
			convertedData.cdaReference &&
		...
		const hasWindAngle =
			convertedData.windAngle && convertedData.windAngle.length > 0;

...
export async function bindGpsDetection(
	...
	const gateSlider = document.getElementById(
		"gpsGateSlider",
	) as HTMLInputElement;
	...
	const resolveGateForSelection = async (
		...
		if (appState.currentFileHash) {
			try {
				const savedMarker = await parameterStorage.loadGpsMarkerSettings(
	...
	const updateGatePosition = async (timeOffset: number, persist = true) => {
		...
		if (lat && lon && lat !== 0 && lon !== 0) {
			...
			if (persist && appState.currentFileHash) {
				try {
					await parameterStorage.saveGpsMarkerSettings(
						appState.currentFileHash,
						appState.selectedLaps,
						{
	...
	gateValue.onchange = () => {
		...
		const val = Math.max(
			0,
			Math.min(parseInt(gateValue.value) || 0, maxSecondsNow),
		);
	...
	callbacks.registerRedetect?.(() => {
		...
		void (async () => {
			...
			if (
				!(await resolveGateForSelection(carry, () => token !== redetectToken))
			)
				return;

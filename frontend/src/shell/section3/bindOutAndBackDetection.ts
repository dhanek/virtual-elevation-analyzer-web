...
export async function bindOutAndBackDetection(
	...
	const gateAValue = document.getElementById(
		"oabGateAValue",
	) as HTMLInputElement;
	const gateAInfo = document.getElementById("oabGateAInfo");
	const gateBSlider = document.getElementById(
		"oabGateBSlider",
	) as HTMLInputElement;
	const gateBValue = document.getElementById(
		"oabGateBValue",
	) as HTMLInputElement;
	...
	const resolveGatesForSelection = async (
		...
		const maxSeconds = Math.floor(
			callbacks.getSelectedDataTimeRange().duration,
		);
		if (maxSeconds <= 0) {
		...
		if (appState.currentFileHash) {
			try {
				...
				if (
					...
					savedMarkers.gateBTimeOffset !== undefined
				) {
					offsetA = savedMarkers.gateATimeOffset;
					offsetB = savedMarkers.gateBTimeOffset;
					log.debug("Loading saved Out and Back gate time offsets:", {
						A: offsetA,
						B: offsetB,
					});
				}
	...
	gateBValue.onchange = () => {
		...
		const val = Math.max(
			minB,
			Math.min(parseInt(gateBValue.value) || 0, maxSecondsNow),
		);
		gateBValue.value = String(val);
		gateBSlider.value = String(val);
		void updateGates();
	...
	callbacks.registerRedetect?.(() => {
		...
		void (async () => {
			...
			if (
				!(await resolveGatesForSelection(carry, () => token !== redetectToken))
			)
				return;

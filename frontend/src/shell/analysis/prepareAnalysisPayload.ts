 ...
 */
import type { PreparedAnalysisSelection, FilteredAnalysisPayload } from '../../modes/analysis/types';
...
export interface PayloadPreparationInput {
    ...
    getNormalizedActivityArrays: (fitData: ActivityDataLike) => NormalizedActivityArrays;
...
export function prepareAnalysisPayload(input: PayloadPreparationInput): PreparedPayload {
    ...
    const selectedIndices = collectSelectionIndices(input.selection, allTimestamps);
    ...
    for (const index of selectedIndices) {
        ...
        filteredAltitude.push(normalized.altitude[index]);
    ...
    if (normalized.cdaReference) {
        filteredCdaReference = selectedIndices.map(index => normalized.cdaReference![index]);
    }

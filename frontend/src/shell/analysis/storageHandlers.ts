...
export async function handleSaveScreenshot(
	...
	const lapCombo =
		appState.currentAnalyzedLaps.length === 0
			? "all"
			: appState.currentAnalyzedLaps.join("-");
...
export async function handleStoreResult(
	...
	if (
		...
		alert(
			"Cannot store result: missing analysis data. Please run analysis first.",
		);
		return;
	}
	...
	if (appState.veStatus !== "ready") {
		alert(
			"Cannot store result: no analysed samples. Please run analysis first.",
		);
		return;
	}
	...
	if (!filteredData || filteredData.power.length === 0) {
		log.error(
			"Cannot store: currentFilteredData missing or empty despite ready status",
		);
		alert(
			"Cannot store result: no analysed samples. Please run analysis first.",
		);
	...
	try {
		...
		if (appState.isGpsLapModeActive) {
			...
			const cdaSlider = document.getElementById(
				"cdaSlider",
			) as HTMLInputElement;
			const crrSlider = document.getElementById(
				"crrSlider",
			) as HTMLInputElement;
			cda = cdaSlider
				? parseFloat(cdaSlider.value)
		...
		if (!Number.isFinite(firstTimestamp)) {
			log.error(`Cannot store: first analysed timestamp is ${firstTimestamp}.`);
			alert(
		...
		const recordingDate = new Date(firstTimestamp * 1000)
			.toISOString()
			.split("T")[0];
 ...
 */
export async function handleShowAllResults(resultsStorage: ResultsViewStorage) {
...
export function bindShowAllResultsButton(
	...
	if (!button) {
		log.warn(
			"Show All Results button not found; the footer entry point is unbound",
		);
		return;
	}
 ...
 * Extracted from main.ts.
 */
export async function handleExportAllResults(resultsStorage: ResultsStorage) {
...
function calculateAverage(
	...
	const validValues = values.filter(
		(v) => !isNaN(v) && (excludeZero ? v !== 0 : true),
	);
...
export function showNotesDialog(): Promise<string | null> {
	return new Promise((resolve) => {
		...
		const okBtn = dialog.querySelector(
			"#notesOkBtn",

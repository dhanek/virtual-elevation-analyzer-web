...
vi.mock("../../modes/analysis/AnalysisModes", () => ({
	collectSelectionIndices: vi.fn(
		(selection: PreparedAnalysisSelection, timestamps: ArrayLike<number>) => {
			if (selection.indexRanges) {
				const indices: number[] = [];
				for (const range of selection.indexRanges) {
					for (
						let i = range.startIdx;
						i <= range.endIdx && i < timestamps.length;
						i++
					) {
						indices.push(i);
					}
				...
				for (let i = 0; i < timestamps.length; i++) {
					if (
						selection.timeRanges.some(
							(range) =>
								timestamps[i] >= range.start && timestamps[i] <= range.end,
						)
					) {
						indices.push(i);
					}
...
function createInput(
	overrides: Partial<PayloadPreparationInput> = {},
...
describe("prepareAnalysisPayload", () => {
	...
	it("throws when filteredTimestamps is empty (no valid data points)", () => {
		...
		expect(() => prepareAnalysisPayload(createInput({ selection }))).toThrow(
			"No valid data points found in selected laps",
		);
	});
});

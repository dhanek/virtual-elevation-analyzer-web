...
export async function updateModeVEPlots(
	...
	const prepared = prepareSegments({
		...
		closure,
		weather: {
			series: appState.weatherSeries,
			calcRho: AirDensityCalculator.calculate_air_density,
		},
	});
	...
	try {
		autoConverge = resolveAutoConvergedControls({
			...
			bounds: {
				cdaMin: params.cda_min ?? DEFAULT_PARAMETERS.cda_min,
				cdaMax: params.cda_max ?? DEFAULT_PARAMETERS.cda_max,
				crrMin: params.crr_min ?? DEFAULT_PARAMETERS.crr_min,
				crrMax: params.crr_max ?? DEFAULT_PARAMETERS.crr_max,
			},
		});
	} catch (err) {
		log.error(
			"Auto-converge failed; using the slider values as they are:",
			err,
		);
		autoConverge = {
			cda: args.cda,
	...
	const segmentWeatherRows = prepared.flatMap((p) =>
		p.weather ? [p.weather] : [],
	);

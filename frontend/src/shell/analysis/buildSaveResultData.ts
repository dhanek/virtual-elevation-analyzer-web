...
export function buildSaveResultData(
	...
	if (
		...
		appState.currentFilteredData.power.length === 0
	) {
		return {
			ok: false,
	...
	const data: SaveResultData = {
		...
		// Per-lap weather as analysed, written by the update pass beside
		// `currentVEResult`. Null when every lap ran on the selection values.

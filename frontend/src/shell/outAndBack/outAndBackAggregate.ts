/**
 * Out-and-back's section mapping and aggregate, extracted verbatim from
 * `updateOutAndBack.ts` (Convergence plan, C4).
 *
 * Same reasoning as `gpsLapAggregate.ts`: `outAndBackMode.summarize` writes
 * this aggregate into `appState.currentVEResult`, so Store Result / Export
 * CSV persist it — and the headless API must reach the same numbers through
 * the same functions, not through a plausible reimplementation.
 */
import {
	matchesRange,
...
import { calculateOutAndBackStats } from "./outAndBackPlots";
 ...
 */
export function toOutAndBackProfiles(
	appState: AppState,
	...
	for (const section of resolveActiveOutAndBackSections(appState)) {
		...
		result.push({
			...
			outboundReferenceElevation: outbound?.referenceElevation ?? null,
			...
			inboundReferenceElevation: inbound?.referenceElevation ?? null,
...
export function computeOutAndBackAggregate(
	profiles: SegmentVeProfile[],
	sectionProfiles: OutAndBackVEProfile[],
	meanElevation: { distances: number[]; elevation: number[] },
): ModeAggregateStats {
	const stats = calculateOutAndBackStats(sectionProfiles, meanElevation);

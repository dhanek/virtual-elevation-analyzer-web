...
describe("renderSelectableCards", () => {
	...
	it("renders correct HTML structure for checked items", () => {
		...
		expect(result).toContain(
			'class="lap-checkbox-item lap-checkbox-item--selected"',
		);
		...
		const items: SelectableCardItem[] = [
			{
				id: "oab-2",
				label: "Section 2",
				details: "Out: 5m • Back: 5m",
				checked: false,
				dataAttr: "oab-section",
				dataValue: 2,
			},
		];
...
describe("bindSelectableCardEvents", () => {
	...
	it("updates selected class on items when checkbox changes", () => {
		...
		expect(cardItem.classList.contains("lap-checkbox-item--selected")).toBe(
			true,
		);

...
import { calculateAirSpeedSyncError, applyAirSpeedOffset } from '../../analysis/WindSourceResolver';
...
export async function updateVEPlotsWithWindSource(
    ...
    if (windSource === 'compare') {
        const constantWindSpeed = new Array(analysisInput.windSpeed.length).fill(NaN);
...
export function setupVESliders(
    ...
    if (!trimStartSlider || !trimEndSlider || !cdaSlider || !crrSlider ||
        !trimStartValue || !trimEndValue || !cdaValue || !crrValue) {
    ...
    const updateTrimStart = () => {
        ...
        if (mapVisualization) {
            mapVisualization.fitBoundsToTrimRegion(value, trimEnd, positionLat, positionLong);
        }
    ...
    const updateTrimEnd = () => {
        ...
        if (mapVisualization) {
            mapVisualization.fitBoundsToTrimRegion(trimStart, value, positionLat, positionLong);
        }
        saveCurrentLapSettings();
    };
    ...
    const updateTrimStartFromInput = () => {
        ...
        if (mapVisualization) {
            mapVisualization.fitBoundsToTrimRegion(clamped, trimEnd, positionLat, positionLong);
        }
        saveCurrentLapSettings();
    };
    ...
    const updateTrimEndFromInput = () => {
        ...
        if (mapVisualization) {
            mapVisualization.fitBoundsToTrimRegion(trimStart, clamped, positionLat, positionLong);
        }
        saveCurrentLapSettings();
    };
    ...
    bindWindSourceRadios(() => {
        ...
        const trimStart = parseInt(trimStartSlider.value);
        const trimEnd = parseInt(trimEndSlider.value);
        updateVEPlotsWithWindSource(appState, analysisInput, trimStart, trimEnd, windSource);
    });

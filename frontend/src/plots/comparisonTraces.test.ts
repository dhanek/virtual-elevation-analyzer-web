...
const EXPECTED_FIT_RESIDUALS = [
	0, 9, 18, 27, 36, 45, 54, 63, 72, 81, 90, 99, 108, 117, 126, 135,
];
/** EXPECTED_CONSTANT - EXPECTED_ACTUAL_MAIN. */
const EXPECTED_CONSTANT_RESIDUALS = [
...
describe("buildVirtualElevationComparisonFigures — frozen trace data (before picture)", () => {
	...
	it("anchors the FIT series on the first main-window actual sample", () => {
		expect(traceNamed(figures.elevation.data, "VE (FIT Air Speed)").y).toEqual(
			EXPECTED_FIT,
		);
	});

	it("anchors the constant series on the SAME sample, independently", () => {
		expect(traceNamed(figures.elevation.data, "VE (Constant Wind)").y).toEqual(
			EXPECTED_CONSTANT,
		);
	});

	it("draws the actual elevation main window unshifted", () => {
		expect(traceNamed(figures.elevation.data, "Actual Elevation").y).toEqual(
			EXPECTED_ACTUAL_MAIN,
		);
	});

	it("takes both residual series against the actual main window", () => {
		expect(
			traceNamed(figures.residuals.data, "Residuals (FIT Air Speed)").y,
		).toEqual(EXPECTED_FIT_RESIDUALS);
		expect(
			traceNamed(figures.residuals.data, "Residuals (Constant Wind)").y,
		).toEqual(EXPECTED_CONSTANT_RESIDUALS);
	});
	...
	it("keeps the line styling of all six traces", () => {
		expect(
			traceNamed(figures.elevation.data, "VE (FIT Air Speed)").line,
		).toEqual({ color: "#4363d8", width: 2 });
		expect(traceNamed(figures.elevation.data, "Actual Elevation").line).toEqual(
			{ color: "#000000", width: 2 },
		);
		expect(
			traceNamed(figures.elevation.data, "VE (Constant Wind)").line,
		).toEqual({ color: "#a9a9a9", width: 2 });
		expect(
			traceNamed(figures.residuals.data, "Residuals (FIT Air Speed)").line,
		).toEqual({ color: "#4363d8", width: 2 });
		expect(
			traceNamed(figures.residuals.data, "Residuals (Constant Wind)").line,
		).toEqual({ color: "#a9a9a9", width: 2 });
		expect(traceNamed(figures.residuals.data, "Zero").line).toEqual({
			color: "#95a5a6",
			width: 1,
			dash: "dash",
		});
...
describe("anchorSeriesTo", () => {
	...
	it("propagates NaN rather than inventing a number", () => {
		...
		expect(anchorSeriesTo([Number.NaN, 1, 2], 0).every(Number.isNaN)).toBe(
			true,
		);
	...
	it("propagates a NaN anchor to every sample", () => {
		expect(anchorSeriesTo([1, 2, 3], Number.NaN).every(Number.isNaN)).toBe(
			true,
		);
	});
});

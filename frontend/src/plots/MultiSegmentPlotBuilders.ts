...
export function buildMultiSegmentWindFigure(
	input: MultiSegmentFigureInput,
): PlotDefinition {
	const traces = input.series.flatMap((series) =>
		createLineTrace({
			x: series.metrics.distancesKm,
			y: series.metrics.apparentWindSpeedMps.map((value) =>
				Number.isFinite(value) ? value * 3.6 : null,
			),
			name: series.label,
			color: series.color,
			dash: series.dash,
...
export function buildMultiSegmentPowerFigure(
	input: MultiSegmentFigureInput,
): PlotDefinition {
	const traces = input.series.flatMap((series) =>
		createLineTrace({
			x: series.metrics.distancesKm,
			y: series.metrics.powerWatts.map((value) =>
...
export function buildMultiSegmentVirtualDistanceFigure(
	input: MultiSegmentFigureInput,
): PlotDefinition {
	const traces = input.series.flatMap((series) => {
		const deltaKm = series.metrics.virtualDistanceAirKm.map((airKm, index) => {
			...
			return Number.isFinite(airKm) && Number.isFinite(groundKm)
				? airKm - groundKm
				: null;
		});
	...
	const maxDistance = Math.max(
		...
		...input.series.map(
			(series) =>
				series.metrics.distancesKm[series.metrics.distancesKm.length - 1] ?? 0,
		),
	);

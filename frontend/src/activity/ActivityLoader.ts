...
export function loadCsvActivity(
	input: LoadCsvActivityInput,
): LoadCsvActivityResult {
...
function normalizeCsvData(csvData: GibliCsvData): {
	...
	if (normalized.temperature)
		dataToInterpolate.temperature = normalized.temperature;
	...
	if (normalized.cdaReference)
		dataToInterpolate.cdaReference = normalized.cdaReference;
	if (normalized.lapNumber) dataToInterpolate.lapNumber = normalized.lapNumber;

	const interpolated = interpolateAllData(
	...
	if (interpolated.temperature)
		normalized.temperature = interpolated.temperature;
	...
	if (interpolated.cdaReference)
		normalized.cdaReference = interpolated.cdaReference;
	...
	normalized.timeRangeSeconds =
		normalized.timestamps.length > 0
			? normalized.timestamps[normalized.timestamps.length - 1] -
...
function buildCsvActivityResult(csvData: GibliCsvData): ActivityResult {
	...
	return {
		...
		parsing_statistics: {
			...
			has_air_speed_data: csvData.airSpeed.some(
...
function generateLapsFromCsv(csvData: GibliCsvData): ActivityLapLike[] {
	...
	const uniqueLapNumbers = Array.from(
		new Set(csvData.lapNumber.filter((n) => !isNaN(n))),
	).sort((a, b) => a - b);
	...
	for (const lapNum of uniqueLapNumbers) {
		...
		laps.push({
			...
			total_elapsed_time:
				csvData.timestamps[endIdx] - csvData.timestamps[startIdx],
...
function createCsvActivityData(csvData: GibliCsvData): ActivityData {
	const distance = calculateDistanceArray(
		csvData.positionLat,
		csvData.positionLong,
	);
	...
	return {
		...
		temperature:
			csvData.temperature || new Array(csvData.timestamps.length).fill(0),

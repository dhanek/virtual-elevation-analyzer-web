...
function validateSelection(
	...
	if (shape === "laps" && !isNumberArray(value)) {
		errors.push({
			path: "selection.laps",
			message: "must be 1-based lap ordinals",
		});
	}
	if (shape === "timeRanges") {
	...
	if (shape === "indexRanges") {
		for (const [i, range] of value.entries()) {
			if (!isRecord(range) || finite(range.startIdx) || finite(range.endIdx)) {
				errors.push({
					path: `selection.indexRanges[${i}]`,
					message: "must be {startIdx, endIdx}",
				});
			}
		}
...
function validateTrim(trim: unknown, errors: RunErrorDetail[]): void {
	...
	if (trim.space === "selection") {
		if (finite(trim.start) || finite(trim.end)) {
			errors.push({
				path: "trim",
				message: "selection-space trim needs {start, end}",
			});
		}
	} else if (trim.space === "segment") {
		if (!isRecord(trim.bySegmentKey)) {
			errors.push({
				path: "trim.bySegmentKey",
				message: "must map segment keys to {start, end}",
			});
			return;
		}
		...
		errors.push({
			path: "trim.space",
			message: "must be 'selection' or 'segment'",
			received: trim.space,
		});
	}
}
		...
		if (!isRecord(raw.parameters)) {
			...
			for (const [key, value] of Object.entries(raw.parameters)) {
				if (
					...
					errors.push({
						path: `parameters.${key}`,
						message: "must be a scalar or null",
					});
				}
			}
		}
		...
		if (!isRecord(raw.activity)) {
			...
			if (inline !== undefined) {
				if (!isRecord(inline) || !isRecord(inline.channels)) {
					errors.push({
						path: "activity.inline",
						message: "must carry {channels}",
					});
				}
			}
		}

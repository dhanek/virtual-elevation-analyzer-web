...
export function serializeRunResult(input: SerializeRunResultInput): RunResult {
	...
	const segments: RunSegmentResult[] = outcome.profiles.map(
		(profile): RunSegmentResult => ({
	...
	if (output.csvRow !== false) {
		...
		if (built.ok) {
			...
			const lw = segmentWeatherCsvCells(row.segmentWeather);
			result.csvRow = {
				headers: RESULT_COLUMNS.map((column) => column.header),
				values: RESULT_COLUMNS.map((column) => column.cell(row, vd, lw)),
	...
	if (output.includeSeries) {
		result.series = {
			bySegment: outcome.profiles.map(
				(profile): NonNullable<RunResult["series"]>["bySegment"][number] => ({
					key: profile.segment.key,

...
const EMPTY_SELECTION_MESSAGE =
	"Please select sections and set parameters first.";
...
export const outAndBackMode: AnalysisModeHandler = {
	...
	prepareSelection(appState): PreparedAnalysisSelection {
		...
		const selectedEntries = appState.outAndBackSections.filter((section) =>
			selectedItems.includes(section.sectionNumber),
		);
	...
	getUpdateSegments(appState): ModeSegment[] {
		return resolveActiveOutAndBackSections(appState).flatMap((section) => [
			{
	...
	summarize(appState, profiles, aggregate, inputs) {
		...
		writeSegmentModeResultState(
			...
			sectionsCoveredByProfiles(sections, profiles).map(

import {
	resolveActiveGpsLapRanges,
	resolveGpsLapNumber,
} from "./activeGpsLapRanges";
 ...
 */
function gpsLapNumberAt(
	appState: AppState,
	range: LapIndexRange,
	index: number,
): number {
	return (
		appState.currentOverlayLapNumbers?.[index] ??
 ...
 */
export function gpsLapNumberForProfile(
	appState: AppState,
	profile: SegmentVeProfile,
	profileIndex: number,
): number {
	return (
		profile.segment.itemNumber ??
	...
	summarize(appState, profiles, aggregate, inputs) {
		writeSegmentModeResultState(
			...
			profiles.map((profile, i) =>
				gpsLapNumberForProfile(appState, profile, i),
			),
		);
	},
};

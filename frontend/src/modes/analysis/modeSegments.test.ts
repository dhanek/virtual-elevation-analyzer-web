...
function makeLap(startTime: number, endTime: number): ActivityLapLike {
	return {
		...
		end_time: endTime,
	} as unknown as ActivityLapLike;
}

function makeDetectedLap(
	lapNumber: number,
	startIdx: number,
	endIdx: number,
): DetectedLap {
...
function makeProfile(indices: number[]): SegmentVeProfile {
	return {
		segment: {
			key: "k",
			label: "l",
			range: { startIdx: indices[0], endIdx: indices[indices.length - 1] },
		},
		indices,
...
describe("gpsLapMode.getUpdateSegments", () => {
	...
	it("falls back to the selected detected laps when no active ranges are set", () => {
		...
		appState.currentFitData = makeFitData();
		appState.gpsDetectedLaps = [
			makeDetectedLap(1, 0, 9),
			makeDetectedLap(2, 10, 19),
		];
...
describe("buildFilteredDataFromProfiles honours the trim window", () => {
	...
	function profileWithTrim(
		...
		return {
			segment: { key: "s", label: "Lap 1", range: { startIdx, endIdx }, trim },
			indices,
		};
	}
...
describe("buildFilteredDataFromProfiles keeps temperature aligned", () => {
	...
	function fullProfile(count: number): any {
		...
		return {
			segment: {
				key: "s",
				label: "Lap 1",
				range: { startIdx: 0, endIdx: count - 1 },
			},
			indices,
		};
	}
	...
	it("emits a NaN per sample rather than an empty array", () => {
		const filtered = buildFilteredDataFromProfiles(
			stateWithoutTemperature(50),
			[fullProfile(50)],
		);
...
describe("standardMode filters unknown lap numbers at the source", () => {
	...
	it("drops it from the update selection too", () => {
		...
		const selection = handler.prepareUpdateSelection!(
			stateWithStaleSelection(),
		);

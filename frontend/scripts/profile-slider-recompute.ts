 ...
 * (`renderStandardVe.ts:148`, `updateModeVEPlots.ts:342-347`). Naming the
...
async function main(): Promise<void> {
	...
	const outAndBackSections = createOutAndBackSections(
		SAMPLE_COUNT,
		OUT_AND_BACK_SECTION_COUNT,
	);

	const scenarios: ScenarioMeasurement[] = [
		await measureScenario("standard-fit", () => {
			runStandardFitRecompute(
				activity,
				params,
				standardTrim.trimStart,
				standardTrim.trimEnd,
			);
		}),
		await measureScenario("standard-compare", () => {
			runStandardCompareRecompute(
				activity,
				params,
				standardTrim.trimStart,
				standardTrim.trimEnd,
	...
	const slowestMedian = Math.max(
		...scenarios.map((scenario) => scenario.medianMs),
	);
...
function runStandardCompareRecompute(
	...
	const constantResult = constantCalculator.calculate_virtual_elevation(
		params.cda,
		params.crr,
		trimStart,
		trimEnd,
	);
	const fitResult = fitCalculator.calculate_virtual_elevation(
		params.cda,
		params.crr,
		trimStart,
		trimEnd,

...
const OUT_AND_BACK_RIDE_PATH = fileURLToPath(
	new URL(
		"../src/analysis/__fixtures__/out-and-back-ride.json",
		import.meta.url,
	),
);
 ...
 */
function deriveDistanceSteps(lapDistance: number[]): {
	outbound: number[];
	inbound: number[];
} {
	const length = lapDistance.length;
	const forward = lapDistance.map((value, i) =>
		i === 0 ? 0 : value - lapDistance[i - 1],
	);
	...
	const inbound = Array.from({ length }, (_, j) =>
		j === 0 ? forward[length - 1] : forward[length - j],
	);

	return { outbound, inbound };
}

export function buildOutAndBackFixture(
	source: GoldenRideSource,
): OutAndBackFixture {
	...
	SOURCE_LAP_NUMBERS.forEach((lapNumber, sectionIndex) => {
		...
		const sliceLap = (values: number[]): number[] =>
			values.slice(range.startIdx, range.endIdx + 1);
		...
		const inboundSourceIdx = (k: number): number =>
			range.startIdx + (length - 1 - k);
		...
		for (let k = 0; k < length; k++) {
			for (const name of COPIED_ARRAYS) {
				copied[name].push(
					round(
						source[name][outboundSourceIdx(k)],
						decimalsFor(name, rounding),
					),
				);
			}
...
function buildProvenance(
	...
	const lapSummary = sourceLaps
		.map(
			(lap) =>
				`lap ${lap.lapNumber} (source indices ${lap.sourceStartIdx}-${lap.sourceEndIdx}, ${lap.length} samples)`,
		)
		.join(", ");
...
function assertFixtureIntegrity(fixture: OutAndBackFixture): void {
	...
	for (const [name, values] of arrays) {
		if (values.length !== fixture.record_count) {
			throw new Error(
				`${name} has ${values.length} samples, record_count is ${fixture.record_count}.`,
			);
		}
	}
	...
	fixture.sections.forEach((section, i) => {
		if (section.inboundStartIdx !== section.outboundEndIdx + 1) {
			throw new Error(
				`section ${section.sectionNumber}: inbound does not start one past outbound.`,
			);
		}
		const outboundLength =
		...
		const expectedStart =
			i === 0 ? 0 : fixture.sections[i - 1].inboundEndIdx + 1;
		if (section.outboundStartIdx !== expectedStart) {
			throw new Error(
				`section ${section.sectionNumber}: starts at ${section.outboundStartIdx}, expected ${expectedStart}.`,
			);
		}
	});
...
function main(): void {
	const source = JSON.parse(
		readFileSync(GOLDEN_RIDE_PATH, "utf8"),
	) as GoldenRideSource;
	...
	const sectionSummary = fixture.sections
		.map(
			(section) =>
				`#${section.sectionNumber} ${section.outboundStartIdx}-${section.inboundEndIdx}`,
		)
		.join("  ");
...
if (
	runner.includes("vite-node") ||
	runner.includes("build-out-and-back-fixture")
) {
	try {
		main();
	} catch (error) {

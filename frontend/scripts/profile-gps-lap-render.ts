...
async function main(): Promise<void> {
	...
	async function measureUpdate(
		...
		appState.selection.currentGpsLapIndexRanges = createSegmentRanges(
			SAMPLE_COUNT,
			lapCount,
		);
		...
		return {
			name: `gps-lap-${lapCount} / ${activeTab}`,
			lapCount,
			samples: phases,
			plotlyCalls: lastCalls,
		};
	}
	...
	process.stdout.write(
		`Samples: ${SAMPLE_COUNT} @ 1 Hz, warmup=${WARMUP_ITERATIONS}, measured=${MEASURED_ITERATIONS}\n`,
	);
	process.stdout.write(
		"Columns: total = whole updateModeVEPlots pass. compute = total minus the\n",
	);
	process.stdout.write(
		"measured callbacks, i.e. the part the existing pre-screen already covers.\n\n",
	);
	process.stdout.write(
		"Scenario                    total(med)  total(p95)   compute   aggregate    renderVe   renderTab\n",
	);
	process.stdout.write(
		"-------------------------------------------------------------------------------------------------\n",
	);
	...
	for (const m of measurements) {
		const summary = m.plotlyCalls
			.map(
				(c) =>
					`${c.method}(${c.divId}) ${c.traceCount} traces / ${c.pointCount} pts`,
			)
			.join("\n      ");
	...
	// ------------------------------------------------- report 1b: double stats
	{
		...
		if (outcome && captured.length > 0) {
			...
			process.stdout.write(
				`  calculateMeanElevationProfile  median ${formatMs(percentile(meanTimings, 0.5))}\n`,
			);
			process.stdout.write(
				`  calculateGpsLapStats           median ${formatMs(percentile(statTimings, 0.5))}\n`,
			);
			process.stdout.write(
				"  Both were rewritten as two-pointer walks (D3), from ~3.8ms and\n",
			);
			process.stdout.write(
				"  ~5.9ms respectively. calculateGpsLapStats no longer runs twice per\n",
			);
			process.stdout.write(
				"  update: renderGpsLapVEPlots takes the aggregate as a parameter (D1),\n",
			);
			process.stdout.write(
				"  and the mean profile is cached across updates (D2).\n",
			);
		}
	}
...
function modelUpdatesDuringDrag(
	...
	const rateCap =
		effectiveSpacing > 0 ? Math.floor(dragMs / effectiveSpacing) : eventCount;
...
async function reportDebounce(
	...
	process.stdout.write(
		`Real runner check: RECOMPUTE_THROTTLE_MS = ${shippedIntervalMs} ms\n`,
	);
	process.stdout.write(
		"This is the number that was 0 across the top three rows under the\n",
	);
	process.stdout.write(
		"resetting debounce -- a frozen plot for the whole drag.\n\n",
	);
	...
	for (const spacing of SPACINGS) {
		...
		const predicted = modelUpdatesDuringDrag(
			shippedIntervalMs,
			spacing,
			DRAG_MS,
		);
		...
		const agree =
			Math.abs(during - predicted) <= Math.max(2, predicted * 0.15)
				? "yes"
				: "NO";
		process.stdout.write(
	...
	const perUpdate = veTab
		? veTab.samples.map((s) => s.total).sort((a, b) => a - b)[
				Math.floor(veTab.samples.length / 2)
			]
		: Number.NaN;
	...
	process.stdout.write(
		"(cost-capped: an update cannot start before the previous one finishes)\n\n",
	);
	process.stdout.write(
		`spacing  ${CANDIDATES.map((c) => `${c}ms`.padStart(8)).join("")}\n`,
	);
	process.stdout.write("-------------------------------------------------\n");
	for (const spacing of SPACINGS) {
		const cells = CANDIDATES.map((d) =>
			String(modelUpdatesDuringDrag(d, spacing, DRAG_MS, perUpdate)).padStart(
				8,
			),
		).join("");
	...
	process.stdout.write(
		"(worst case: the input lands just after a run starts, so it waits out\n",
	);
	...
	for (const d of CANDIDATES) {
		process.stdout.write(
			`${`${d}ms`.padStart(8)}   ${formatMsLocal(d + perUpdate).padStart(28)}\n`,
		);
	}
	...
	for (const d of CANDIDATES) {
		...
		process.stdout.write(
			`  ${`${d}ms`.padStart(5)}  ${(duty * 100).toFixed(0)}% busy, ${(1000 / effective).toFixed(0)} updates/sec\n`,
		);
	}

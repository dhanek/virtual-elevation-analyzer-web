...
async function main(): Promise<void> {
	...
	const buildCalculator = (
		startIdx: number,
		endIdx: number,
	): { calculator: VirtualElevationCalculator; sampleCount: number } => {
	...
	const gpsLaps = createSegmentRanges(SAMPLE_COUNT, GPS_LAP_COUNT).map(
		(range) => buildCalculator(range.startIdx, range.endIdx),
	);

	const scenarios: ScenarioMeasurement[] = [];
	for (const steps of RESOLUTIONS) {
		scenarios.push(
			await measureScenario(`standard-1seg ${steps}x${steps}`, () => {
				runGrid(standard, steps);
			}),
		);
		scenarios.push(
			await measureScenario(
				`gps-lap-${GPS_LAP_COUNT}seg ${steps}x${steps}`,
				() => {
	...
	scenarios.push(
		await measureScenario(
			`bisection ${BISECTION_CALLS}x ve_gain, ${GPS_LAP_COUNT}seg`,
			() => {
	...
	process.stdout.write(
		`Samples: ${SAMPLE_COUNT} @ 1 Hz; standard window ${standardSamples}; `,
	);
	process.stdout.write(
		`${GPS_LAP_COUNT} laps of ~${Math.round(lapSamples.reduce((a, b) => a + b, 0) / lapSamples.length)}\n`,
	);
	process.stdout.write(
	...
	process.stdout.write("\nDecision guide:\n");
	process.stdout.write(
		"- The default resolution is the largest whose worst realistic scenario stays under ~300 ms.\n",
	);
	process.stdout.write(
		"- If nothing does, the next lever is the algebraic sin(atan(x)) = x/sqrt(1+x^2) in GainKernel::gain,\n",
	);
	process.stdout.write(
		"  behind a Rust test proving agreement with ve_elevation_diff — not a worker (deferred per PROJECT_STATUS.md).\n",
	);
}

function runGrid(
	segments: Array<{
		calculator: VirtualElevationCalculator;
		sampleCount: number;
	}>,
	steps: number,
): void {
	for (const { calculator, sampleCount } of segments) {
		...
		if (grid.length !== steps * steps) {
			throw new Error(
				`grid came back ${grid.length} cells, expected ${steps * steps}`,
			);
		}
	}
}

function runBisectionCost(
	segments: Array<{
		calculator: VirtualElevationCalculator;
		sampleCount: number;
	}>,

...
function loadActivityFor(
	...
	throw new Error(
		"no activity: pass --file, activity.path, or activity.inline.channels",
	);
}

function loadFromPath(
	path: string,
	declaredType?: "fit" | "csv",
): LoadedRunActivity {
	const type =
		declaredType ?? (extname(path).toLowerCase() === ".csv" ? "csv" : "fit");
	return type === "csv"
		? loadRunActivity({
				kind: "csv",
...
async function main(): Promise<void> {
	...
	const configText =
		configArg === "-" ? await readStdin() : readFileSync(configArg, "utf8");
	const configDir =
		configArg === "-" ? process.cwd() : dirname(resolve(configArg));
	...
	try {
		...
		emit(
			envelope(
				"invalid-config",
				`config is not JSON: ${error instanceof Error ? error.message : error}`,
			),
		);
		process.exitCode = 2;
		return;

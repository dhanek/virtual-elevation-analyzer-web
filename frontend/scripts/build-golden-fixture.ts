...
} from "../src/components/AnalysisParameters";
...
export interface AnonymisationRecord {
	...
	sampleWindow: {
		 ...
		 */
		selection:
			"explicit-lap-range" | "explicit-lap" | "explicit-index" | "auto";
...
async function main(): Promise<void> {
	...
	process.stdout.write(
		`  duration      : ${fixture.timestamps[fixture.record_count - 1].toFixed(0)} s\n`,
	);
	...
	process.stdout.write(
		`  rho min/max   : ${Math.min(...fixture.rhoArray).toFixed(6)} / ${Math.max(...fixture.rhoArray).toFixed(6)}\n\n`,
	);
	process.stdout.write(
		"Open the review page in a browser before approving. Nothing has been\n",
	);
...
export function anonymise(
	...
	const slice = <T>(values: T[]): T[] =>
		values.slice(window.startIdx, window.endIdx + 1);
	...
	for (let i = 0; i < count; i++) {
		...
		position_lat.push(
			round(origin.lat + dLat * cosT - dLong * sinT, COORD_DECIMALS),
		);
		position_long.push(
			round(origin.long + dLat * sinT + dLong * cosT, COORD_DECIMALS),
		);
	}
	...
	const altitudeOffset =
		randomAltitudeOffsetMetres() - round(mean(altitudeRaw), ALTITUDE_DECIMALS);
	const altitude = altitudeRaw.map((a) =>
		round(a + altitudeOffset, ALTITUDE_DECIMALS),
	);

	// Step 4 (everything else): round to the sensor's own precision.
	const power = slice(source.power).map((v) => round(v, POWER_DECIMALS));
	const velocity = slice(source.velocity).map((v) =>
		round(v, VELOCITY_DECIMALS),
	);
	const air_speed = slice(source.air_speed).map((v) =>
		round(v, OTHER_DECIMALS),
	);
	const wind_yaw = slice(source.wind_yaw).map((v) => round(v, OTHER_DECIMALS));
	const temperature = slice(source.temperature).map((v) =>
		round(v, OTHER_DECIMALS),
	);
	...
	const distance = distanceRaw.map((d) =>
		round(d - distanceBase, OTHER_DECIMALS),
	);
	...
	const laps = rawLaps
		...
		.filter(
			(lap) =>
				lap.end_time > timestamps[0] && lap.start_time < timestamps[count - 1],
		);
	...
	const _anonymisation: AnonymisationRecord = {
		...
		sampleWindow: {
			...
			...(options.windowLaps !== null
				? {
						sourceLaps: {
							first: options.windowLaps.first,
							last: options.windowLaps.last,
						},
					}
				: {}),
		},
...
function buildRhoArray(temperature: number[], count: number): number[] {
	...
	const hasTemperature =
		temperature.length === count &&
		temperature.some((t) => Number.isFinite(t) && t !== 0);

	for (let i = 0; i < count; i++) {
		const ramp = i / Math.max(1, count - 1);
		const pressureHpa = 1013 - ramp * 6;
		const tempC =
			hasTemperature && Number.isFinite(temperature[i])
				? temperature[i]
				: 15 + ramp * 4;
		const humidity = 55;
		let value: number;
		try {
			value = AirDensityCalculator.calculate_air_density_from_humidity(
				tempC,
				pressureHpa,
				humidity,
			);
...
function chooseWindow(
	...
	if (options.windowStart !== null) {
		const startIdx = Math.max(
			0,
			Math.min(options.windowStart, sourceCount - length),
		);
		return { startIdx, endIdx: startIdx + length - 1 };
	}
	...
	for (let startIdx = 0; startIdx + length <= sourceCount; startIdx += step) {
		...
		const boundaries = boundaryIndices.filter(
			(i) => i > startIdx && i < endIdx,
		).length;
		const dropouts = dropoutIndices.filter(
			(i) => i >= startIdx && i <= endIdx,
		).length;
...
function buildSections(
	...
	return indexRanges.map((range, i) => {
		const midpoint =
			range.startIdx + Math.floor((range.endIdx - range.startIdx) / 2);
		return {
			sectionNumber: i + 1,
...
export function buildReviewHtml(fixture: GoldenRideFixture): string {
	const routePath = buildSvgPath(
		fixture.position_long,
		fixture.position_lat,
		760,
		420,
		true,
	);
	...
	const rows: Array<[string, string]> = [
		...
		[
			"Altitude range (m)",
			`${altExtent.min.toFixed(1)} … ${altExtent.max.toFixed(1)} (${(altExtent.max - altExtent.min).toFixed(1)} m relief)`,
		],
		...
		[
			"Source window",
			`${fixture._anonymisation.sampleWindow.startIdx}…${fixture._anonymisation.sampleWindow.endIdx} of ${fixture._anonymisation.sampleWindow.sourceRecordCount} (${fixture._anonymisation.sampleWindow.selection})`,
		],
		[
			"Source laps",
...
function buildSvgPath(
	...
	for (let i = 0; i < xs.length; i++) {
		...
		const py =
			height - pad - ((ys[i] - yExtent.min) / ySpan) * (height - 2 * pad);
...
function parseArgs(argv: string[]): Options {
	...
	if (windowLapsRaw !== null) {
		...
		if (!match) {
			throw new Error(
				`--window-laps must look like "8-14" (got "${windowLapsRaw}").`,
			);
		}
		...
		if (first < 1 || last < first) {
			throw new Error(
				`--window-laps=${windowLapsRaw} is not a valid 1-based inclusive range.`,
			);
		}
	...
	if (
		windowLaps === null &&
		(!Number.isFinite(windowLength) ||
			windowLength < MIN_WINDOW ||
			windowLength > MAX_WINDOW)
	) {
		throw new Error(
			`--window must be between ${MIN_WINDOW} and ${MAX_WINDOW} (got ${windowLength}).`,
		);
	}
...
if (positionalArgs.length > 0) {
	...
	process.argv[1]?.includes("build-golden-fixture")
) {
